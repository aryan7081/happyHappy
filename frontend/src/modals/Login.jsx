...
const Login = ({ closeModals, openRegisterModal }) => {
  ...
  const { isSubscribedMember, checkMembershipStatus } =
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.apiBaseUrl}/api/login/`, payload);
      const data = await response.json();

      if (response.ok) {
        localStorage.setItem('user', JSON.stringify(data));
        login(data);
        isSubscribedMember ? navigate('/dashboard') : navigate('/home');
      } else {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl p-8 shadow-2xl border border-gray-200 relative">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              className="text-sm text-blue-600 hover:text-blue-700 font-medium bg-transparent border-0 p-0 cursor-pointer"
            >
              Forgot password?
            </button>
          </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button
            type="button"
            className="w-full inline-flex justify-center py-3 px-4 border border-gray-200 rounded-xl bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors duration-200"
          >
